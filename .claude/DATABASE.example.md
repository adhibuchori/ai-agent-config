...
> firewall rule. Written assuming `pgvector` — if your service has no vector column, delete that
> one sentence in Topology and the rest applies unchanged.
...
| Database          | Owner role      | `pgvector` extension | Used by repo |
| ------------------ | --------------- | --------------------- | ------------ |
| `<database-name>` | `<owner-role>`  | `<enabled / no>`      | `<repo>`     |

If more than one repo shares this database with different roles (a writer and a reader, say),
name both here and say which is which — that distinction is easy to get backwards from inside a
single repo's context.
...
- `mcp__db-prod__*` — read and diagnostic tools may be pre-approved (schema listing, query
  explain, index/health analysis). **`execute_sql` must not be**, so every write against
  production still raises a permission prompt.
